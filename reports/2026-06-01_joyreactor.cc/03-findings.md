...
### Два рівні фільтрації (виявлено під час тестування)

Аналіз відповідей показав два різні рівні обробки URL:

| Error | Рівень | Протоколи |
|-------|--------|-----------|
| `"Picture(s) failed to upload."` (без extensions) | PHP/Laravel — до FFmpeg | `gopher://`, `dict://`, `ftp://` |
| `"Error probing /tmp/.../uploaded.image via FFMpeg"` | FFmpeg — файл завантажено в `/tmp/` | `http://`, `https://`, `file://`, IPv6, IP encoding |

`gopher://`, `dict://`, `ftp://` блокуються **до** FFmpeg на рівні валідації URL в PHP.  
`file://` та всі HTTP-варіанти **доходять до FFmpeg** — файл реально читається і зберігається в `/tmp/`.

### file:// через HTTP redirect (підтверджено)

FFmpeg слідує HTTP 302 редіректам, в тому числі до `file://` URI. Перевірено через зовнішній сервер (Cloudflare Tunnel):

```
GET https://attacker.com/redirect/file
→ 302 Location: file:///etc/passwd
→ FFmpeg читає /etc/passwd → зберігає в /tmp/XXX/uploaded.image → "Error probing"
```

Підтверджено для: `/etc/passwd`, `/etc/hosts`, `/var/www/html/.env`, `/proc/self/environ`.  
Контент файлу **не відображається у відповіді** — FFmpeg читає але не може декодувати текст як image.

### Внутрішній port scan (уточнені результати)

| Порт | Сервіс | Час (прямий) | Час (через redirect) | Статус |
|------|--------|-------------|---------------------|--------|
| 9999 | baseline закритий | ~256ms | ~500ms | закритий |
| 80 | nginx/apache | ~249ms | ~500ms | закритий |
| 6379 | Redis | ~263ms | ~600ms | **відкритий** (швидка бінарна відповідь) |
| 3306 | MySQL | ~260ms | ~600ms | **відкритий** (швидка бінарна відповідь) |
| 3000 | невідомий сервіс | **15sec timeout** | **15sec timeout** | **ВІДКРИТИЙ — non-HTTP** |
| 3001 | невідомий сервіс | **15sec timeout** | **15sec timeout** | **ВІДКРИТИЙ — non-HTTP** |

**Port 3000/3001:** приймає TCP-з'єднання, але не відповідає на HTTP GET жодним шляхом (`/`, `/metrics`, `/api/health`, `/info`, `/version`, `/status`, `/debug/vars`). Характерно для non-HTTP протоколу (gRPC, кастомний RPC, або сервіс що чекає client-speaks-first).

**Redis/MySQL:** відповідають одразу бінарним banner'ом — підтверджено що сервіси запущені та доступні з localhost.

**RFC1918 підмережі** (`10.x`, `172.16.x`, `172.17.x`, `192.168.x`) — ~5sec timeout = не маршрутуються (окрім Docker bridge `172.17.0.1` — однаковий результат).

### Обмеження поточного вектору

FFmpeg запускається в режимі `ffprobe` (визначення формату) — контент файлу не передається назад у відповіді. Прямий шлях до exfiltration через цей вектор не знайдено.

### Виправлення

- Валідувати URL перед завантаженням — дозволяти тільки `http`/`https` з публічними IP
- **Заборонити HTTP redirects у FFmpeg** (`-protocol_whitelist` без `file`, `-follow_redirects 0`)
...
- Розглянути sandboxing FFmpeg (seccomp, network namespace)
